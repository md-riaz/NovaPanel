...
### Sudo Configuration
Limited sudo access is granted only for essential system operations. The sudoers file is automatically generated by the `scripts/setup-sudoers.sh` script with paths that match your system.
...
The script automatically detects the correct paths for system commands (e.g., `/usr/bin/mkdir` vs `/bin/mkdir`) to ensure compatibility across different Linux distributions.

**Example configuration** (paths may vary based on your system):
```
...
novapanel ALL=(ALL) NOPASSWD: /usr/bin/systemctl reload nginx
novapanel ALL=(ALL) NOPASSWD: /usr/bin/systemctl reload php*-fpm
novapanel ALL=(ALL) NOPASSWD: /usr/bin/systemctl reload bind9
novapanel ALL=(ALL) NOPASSWD: /usr/bin/systemctl reload named

# File and directory operations
novapanel ALL=(ALL) NOPASSWD: /usr/bin/mkdir
novapanel ALL=(ALL) NOPASSWD: /usr/bin/chown
novapanel ALL=(ALL) NOPASSWD: /usr/bin/chmod
novapanel ALL=(ALL) NOPASSWD: /usr/bin/ln
novapanel ALL=(ALL) NOPASSWD: /usr/bin/rm
novapanel ALL=(ALL) NOPASSWD: /usr/bin/cp
novapanel ALL=(ALL) NOPASSWD: /usr/bin/mv
...
novapanel ALL=(ALL) NOPASSWD: /usr/bin/bash
```

**Note:** This configuration is automatically created by the `install.sh` script or `scripts/setup-sudoers.sh`. Only manual configuration is needed if you're performing a manual installation or if you need to update an existing installation.

**Troubleshooting:** If you see "Sudo configuration is missing or incomplete" errors after already configuring sudoers, re-run the setup script to regenerate the configuration with the correct paths for your system:
```bash
sudo bash /opt/novapanel/scripts/setup-sudoers.sh
```
